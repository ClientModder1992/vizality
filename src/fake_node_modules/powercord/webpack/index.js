...
class Webpack {
  ...
  // Just a shorthand alternative version of getModule
  static getMdl (...props) {
    return Webpack.getModule(props, false);
  }
   ...
   * i.e. `getMdl('x', 'y', 'z').x()` is equivalent to `get('x', 'y', 'z')()`
   */
  static get (...props) {
    const potentialModule = Webpack.getModule(props, false);
    if (potentialModule) {
      const extracted = potentialModule[props[0]];
      if (typeof extracted === 'function') {
        return extracted.bind(potentialModule);
      }
      return extracted;
    }
    return null;
