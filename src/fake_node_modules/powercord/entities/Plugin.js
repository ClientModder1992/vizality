...
const navigate = require('powercord/navigate');
...
class Plugin extends Updatable {
  ...
  async _load () {
    try {
      ...
      if (typeof this.navigatedTo === 'function') {
        for (const route in navigate.routes) {
          navigate.on(route, () => this.navigatedTo(route));
        }
        navigate.on('channel', () => this.navigatedTo('channel'));
      }
