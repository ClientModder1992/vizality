...
module.exports = class StyleManager {
  constructor () {
    ...
    this.manifestKeys = [ 'name', 'version', 'description', 'author' ];
  }
  ...
  _validateManifest (manifest) {
    ...
    if (manifest.source && typeof manifest.source !== 'string') {
      errors.push(`Invalid source: expected a string, but received ${typeof manifest.source}`);
      if (!manifest.website.startsWith('^(https://(www.)?github.com/)')) {
        errors.push('Invalid source: source website address must start with https://github.com/ or https://www.github.com/');
      }
    }
    if (manifest.website && typeof manifest.website !== 'string') {
      errors.push(`Invalid website: expected a string, but received ${typeof manifest.website}`);
      if (!manifest.website.startsWith('^((http|https|ftp)://)')) {
        errors.push('Invalid website: website address must start with http:// or https://');
      }
    }
