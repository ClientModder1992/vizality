...
/**
 * @property {VizalityExperiment[]} experiments
 */
class LabsAPI extends API {
  ...
  isExperimentEnabled (experimentId) {
    return vizality.settings.get('labs', []).includes(experimentId);
  }
  ...
  enableExperiment (experimentId) {
    ...
    vizality.settings.set('labs', [
      ...vizality.settings.get('labs', []),
      experimentId
    ]);
  ...
  disableExperiment (experimentId) {
    ...
    vizality.settings.set('labs', vizality.settings.get('labs', []).filter(e => e !== experimentId));
    if (experiment.callback) {
