...
module.exports = class Translate extends Plugin {
  async startPlugin () {
    ...
    vizality.api.settings.registerSettings('vz-translate', {
  ...
  pluginWillUnload () {
    ...
    vizality.api.settings.unregisterSettings('vz-translate');
    uninject('vz-translate-icon');
    uninject('vz-translate-slateContext');
    uninject('vz-translate-context');
    uninject('vz-translate-content');
    uninject('vz-translate-contentRemove');
    uninject('vz-translate-clearRestore');
  }
  ...
  async _injectTranslator () {
    ...
    inject('vz-translate-content', MessageContent.prototype, 'render', function (args) {
      const { message, message: { embeds } } = this.props;
      const embed = embeds.length > 0 ? embeds[0] : null;
    ...
    inject('vz-translate-slateContext', SlateContextMenu, 'default', (args, res) => {
      ...
      const setText = async (opts) => {
        ...
        if (document.getElementById('vizality-translate-resetButton')) {
          return;
        }

        const textAreaButtons = document.getElementsByClassName(classes.buttons)[uploadModal !== null ? 1 : 0];
        const buttonContainer = createElement('div', { id: 'vizality-translate-resetButton',
      ...
      const setText = async (opts) => {
        ...
        if (!timestamp.innerHTML.includes('Translated from')) {
          const translateReset = createElement('span', {
            ...
            className: 'vizality-translate-reset',
            async onclick () {
              message.style.opacity = '0';
              await sleep(200);
          ...
          if (cozy) {
            ...
            const translateResetContainer = createElement('div', {
              className: 'vizality-translate-reset-compact-container'
            });
  ...
  removeResetButton () {
    ...
    const resetButton = document.getElementById('vizality-translate-resetButton');
    if (resetButton) {
      resetButton.remove();
    }
  }
